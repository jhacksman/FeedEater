...
import { BitstampIngestor, parseBitstampSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_bitstamp: {
        async stream({ ctx }) {
          const raw = await ctx.fetchInternalSettings("bitstamp");
          const settings = parseBitstampSettingsFromInternal(raw);
          if (!settings.enabled) {
            return {
              metrics: {
                skipped: true,
          ...
          const ingestor = new BitstampIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
