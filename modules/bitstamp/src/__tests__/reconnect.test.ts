...
describe("Bitstamp WebSocket Reconnection", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  it("should have reconnect properties initialized", async () => {
    ...
    const ingestor = new (mod as any).BitstampIngestor(settings, db, nats, sc, opts);
  ...
  it("should trip circuit breaker after 10 failed reconnects", async () => {
    ...
    const deadCall = nats.publish.mock.calls.find(
      (c: any[]) => c[0] === "feedeater.module.dead.bitstamp"
    );
  ...
  it("should emit reconnecting event via NATS", async () => {
    ...
    const reconnectingEvent = logs.some((l) => l.includes('"module":"bitstamp"') && l.includes('"attempt":1'));
