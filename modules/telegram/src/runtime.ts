...
import { TelegramIngestor, parseTelegramSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_telegram: {
        async collect({ ctx }) {
          const raw = await ctx.fetchInternalSettings("telegram");
          const settings = parseTelegramSettingsFromInternal(raw);
          if (!settings.enabled) {
          ...
          const ingestor = new TelegramIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
          ...
          return {
            metrics: {
              ...
              dialogs_processed: result.dialogsProcessed,
            },
          };
        },
