...
import { DiscordIngestor, parseDiscordSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_discord: {
        async collect({ ctx }) {
          const raw = await ctx.fetchInternalSettings("discord");
          const settings = parseDiscordSettingsFromInternal(raw);
          ...
          const ingestor = new DiscordIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
