...
import { BitfinexIngestor, parseBitfinexSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_bitfinex: {
        async stream({ ctx }) {
          const raw = await ctx.fetchInternalSettings("bitfinex");
          const settings = parseBitfinexSettingsFromInternal(raw);
          if (!settings.enabled) {
            return {
              metrics: {
                skipped: true,
          ...
          const ingestor = new BitfinexIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
