...
import { BlueskyIngestor, parseBlueskySettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_bsky: {
        async collect({ ctx }) {
          const raw = await ctx.fetchInternalSettings("bluesky");
          const settings = parseBlueskySettingsFromInternal(raw);
          if (!settings.enabled) {
            return {
              metrics: {
                skipped: true,
          ...
          const ingestor = new BlueskyIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
          ...
          return {
            metrics: {
              posts_seen: result.insertedOrUpdated,
              posts_published: result.publishedNew,
            },
          };
        },
