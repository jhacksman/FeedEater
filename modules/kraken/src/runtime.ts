...
import { KrakenIngestor, parseKrakenSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_kraken: {
        async stream({ ctx }) {
          const raw = await ctx.fetchInternalSettings("kraken");
          const settings = parseKrakenSettingsFromInternal(raw);
          if (!settings.enabled) {
            return {
              metrics: {
                skipped: true,
          ...
          const ingestor = new KrakenIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
