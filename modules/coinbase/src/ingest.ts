...
export class CoinbaseIngestor {
  ...
  private scheduleReconnect(): void {
    ...
    if (this.reconnectAttempts >= 10) {
      ...
      this.log("error", "module coinbase circuit breaker tripped after 10 failed reconnects", {
        attempts: this.reconnectAttempts,
      });
      this.nats.publish(
        "feedeater.module.dead.coinbase",
        this.sc.encode(JSON.stringify({ module: "coinbase", timestamp: new Date().toISOString(), reason: "circuit breaker tripped after 10 failed reconnects" }))
      );
      return;
    }
