...
describe("Coinbase WebSocket Reconnection", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  it("should have reconnect properties initialized", async () => {
    ...
    const ingestor = new (mod as any).CoinbaseIngestor(settings, db, nats, sc, opts);
  ...
  it("should emit reconnecting event via NATS", async () => {
    ...
    const reconnectingEvent = logs.some((l) => l.includes('"module":"coinbase"') && l.includes('"attempt":1'));
