...
import { CoinbaseIngestor, parseCoinbaseSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_coinbase: {
        async stream({ ctx }) {
          const raw = await ctx.fetchInternalSettings("coinbase");
          const settings = parseCoinbaseSettingsFromInternal(raw);
          if (!settings.enabled) {
            return {
              metrics: {
                skipped: true,
          ...
          const ingestor = new CoinbaseIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
