...
import { GeminiIngestor, parseGeminiSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_gemini: {
        async stream({ ctx }) {
          const raw = await ctx.fetchInternalSettings("gemini");
          const settings = parseGeminiSettingsFromInternal(raw);
          if (!settings.enabled) {
            return {
              metrics: {
                skipped: true,
          ...
          const ingestor = new GeminiIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
