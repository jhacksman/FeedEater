...
export class GeminiIngestor {
  ...
  private scheduleReconnect(): void {
    ...
    if (this.reconnectAttempts >= 10) {
      ...
      this.log("error", "module gemini circuit breaker tripped after 10 failed reconnects", {
        attempts: this.reconnectAttempts,
      });
      this.nats.publish(
        "feedeater.module.dead.gemini",
        this.sc.encode(JSON.stringify({ module: "gemini", timestamp: new Date().toISOString(), reason: "circuit breaker tripped after 10 failed reconnects" }))
      );
      return;
    }
