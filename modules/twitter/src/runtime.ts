...
import { TwitterIngestor, parseTwitterSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_twitter: {
        async collect({ ctx }) {
          const raw = await ctx.fetchInternalSettings("twitter");
          const settings = parseTwitterSettingsFromInternal(raw);
          if (!settings.enabled) {
            return { metrics: { skipped: true, reason: "module disabled" } };
          }
          ...
          const ingestor = new TwitterIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
          ...
          return {
            metrics: {
              tweets_seen: result.insertedOrUpdated,
              tweets_published: result.publishedNew,
            },
          };
        },
