...
import { MastodonIngestor, parseMastodonSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_mastodon: {
        async collect({ ctx }) {
          const raw = await ctx.fetchInternalSettings("mastodon");
          const settings = parseMastodonSettingsFromInternal(raw);
          ...
          const ingestor = new MastodonIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
          ...
          return {
            metrics: {
              toots_seen: result.insertedOrUpdated,
              toots_published: result.publishedNew,
              boosts_skipped: result.boostsSkipped,
            },
          };
        },
