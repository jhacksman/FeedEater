...
export class OkxIngestor {
  ...
  private scheduleReconnect(): void {
    ...
    if (this.reconnectAttempts >= 10) {
      ...
      this.log("error", "module okx circuit breaker tripped after 10 failed reconnects", {
        attempts: this.reconnectAttempts,
      });
      this.nats.publish(
        "feedeater.module.dead.okx",
        this.sc.encode(JSON.stringify({ module: "okx", timestamp: new Date().toISOString(), reason: "circuit breaker tripped after 10 failed reconnects" }))
      );
      return;
    }
