...
import { OkxIngestor, parseOkxSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_okx: {
        async stream({ ctx }) {
          const raw = await ctx.fetchInternalSettings("okx");
          const settings = parseOkxSettingsFromInternal(raw);
          if (!settings.enabled) {
            return {
              metrics: {
                skipped: true,
          ...
          const ingestor = new OkxIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
