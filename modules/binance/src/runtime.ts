...
import { BinanceIngestor, parseBinanceSettingsFromInternal } from "./ingest.js";
...
export function createModuleRuntime(): ModuleRuntime {
  return {
    ...
    handlers: {
      mod_binance: {
        async stream({ ctx }) {
          const raw = await ctx.fetchInternalSettings("binance");
          const settings = parseBinanceSettingsFromInternal(raw);
          if (!settings.enabled) {
            return {
              metrics: {
                skipped: true,
          ...
          const ingestor = new BinanceIngestor(settings, ctx.db, ctx.nats, ctx.sc, {
            apiBaseUrl,
            internalToken,
