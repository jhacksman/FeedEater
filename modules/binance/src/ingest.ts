...
export class BinanceIngestor {
  ...
  private scheduleReconnect(): void {
    ...
    if (this.reconnectAttempts >= 10) {
      ...
      this.log("error", "module binance circuit breaker tripped after 10 failed reconnects", {
        attempts: this.reconnectAttempts,
      });
      this.nats.publish(
        "feedeater.module.dead.binance",
        this.sc.encode(JSON.stringify({ module: "binance", timestamp: new Date().toISOString(), reason: "circuit breaker tripped after 10 failed reconnects" }))
      );
      return;
    }
