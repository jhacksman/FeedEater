...
import { DataSchemaStore, getModuleDataSchema } from "./moduleDataSchema.js";
...
const natsHealthStore = new NatsHealthStore();
...
app.post("/api/modules/:name/reconnect", postModuleReconnect({ getNatsConn, sc: natsSc, tracker: reconnectTracker }));
...
app.get("/api/modules/:name/data-schema", getModuleDataSchema({ schemaStore: dataSchemaStore }));
...
app.get("/api/system/nats-health", getSystemNatsHealth({ natsHealthStore }));
